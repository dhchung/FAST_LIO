...
void Preprocess::hesai_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  if (feature_enabled)
  {
    ...
    for (uint i = 0; i < plsize; i++)
    {
      ...
      added_pt.curvature = (pl_orig.points[i].timestamp - init_time) * time_unit_scale;
      if(pl_orig.points[i].ring < N_SCANS)
      {
        pl_buff[pl_orig.points[i].ring].push_back(added_pt);
      }
  ...
  else
  {
    ...
    for (int i = 0; i < pl_orig.points.size(); i++)
    {
      ...
      added_pt.normal_y = 0;
      added_pt.normal_z = 0;
      added_pt.curvature = (pl_orig.points[i].timestamp - init_time) * time_unit_scale; // curvature unit: ms
